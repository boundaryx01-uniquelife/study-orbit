...
server.listen(PORT, () => {
  console.log(`STUDY ORBIT v0.5-webdb running at http://localhost:${PORT}`);
...
async function handleBookSearch(url, res) {
  ...
  if (!response.ok) {
    ...
    sendJson(res, response.status, { error: "KAKAO_BOOK_API_ERROR", status: response.status, detail: detail.slice(0, 500) });
    return;
  }
